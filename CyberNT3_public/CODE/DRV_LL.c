...
static void SetRatio(void)
{
  setReg(PWMB_PWMVAL2,((getReg(PWMB_PWMCM)+1) * (dword)RatioStore) >> 16); /* Calculate new value according to the given ratio */
...
void DRV_LL_Init(void)
{
  ...
  setRegBits(PWMB_PMCFG,512);          /* Set polarity in the PWM configure register */
  setReg(PWMB_PWMVAL2,2000);           /* Store initial value to the duty-compare register */
