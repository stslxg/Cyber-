...
def CalcCanny():
  ...
  for i in xrange(0,ROW,1):
    ...
    for j in xrange(COL-1,1,-1):
      #canny3[i][j] = (canny2[i][j]-canny2[i][j-1])*(canny2[i][j] - canny2[i][j-1])
      canny3[i][j] = abs(canny2[i][j] - canny2[i][j-1])*3 
      #if canny3[i][j] > maxV:
      #  maxV = canny3[i][j]
      #if canny3[i][j] < minV:
      #  minV = canny3[i][j]
  ...
  #    canny[i][j] = (canny3[i][j] - minV) * 255.0 / (maxV - minV)
  #    if canny3[i][j] > WHITE:
  #      canny3[i][j] = WHITE
  #for i in xrange(1,ROW-1,1):
  #  for j in xrange(1,COL-1,1):
  #    if (canny3[i][j] >= 160) and (canny3[i][j] >= canny3[i][j-1]) and (canny3[i][j] >= canny3[i][j+1]):
  #      canny[i][j] = WHITE
  #    else :
  #      canny[i][j] = canny3[i][j]
