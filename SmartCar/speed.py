...
def saveFile(win, event):
  global count
  if event.keyval == ord('s'):
    print "Saving..."

    d = shelve.open('speed%d.txt' % (count,), flag = 'c', writeback = True)
    d['left'] = deepcopy(left)
    d['right'] = deepcopy(right)
    d['leftv'] = deepcopy(leftv)
    d['rightv'] = deepcopy(rightv)
    d['leftPWM'] = deepcopy(leftPWM)
    d['rightPWM'] = deepcopy(rightPWM)
    d['servo'] = deepcopy(servo)
    d['position'] = deepcopy(pos)
    d.close()
...
win.connect('key-press-event', saveFile)
win.add(canvas)
